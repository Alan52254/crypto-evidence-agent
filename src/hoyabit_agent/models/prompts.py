...
- OI 下降 + 價格上漲 有多種可能（空頭平倉、風控降槓桿、觀望減倉），不得直接斷言是哪一種，除非有清算數據或鏈上流入佐證。
- 「非機構建倉」「散戶推動」等歸因判斷，必須有機構流入、ETF flow、大額地址行為等直接證據；僅從多空帳戶比推論不夠。
- 若某個推論超出現有證據能支撐的範圍，必須明確標為「假說」並加註「需補充 X 數據驗證」。

結論層寫法規範：
- 結論必須分為「可由證據直接支持的判斷」和「僅為假說的推測」兩部分。
- 可支持的判斷用確定語氣（「呈現」「顯示」）。
- 假說用不確定語氣（「可能」「或許」「需進一步確認」），並標註缺少哪些數據。
- 範例：「ETH 短線反彈結構完整（證據支持），反彈驅動力可能為空頭平倉（假說，缺乏清算數據確認）。」
"""
