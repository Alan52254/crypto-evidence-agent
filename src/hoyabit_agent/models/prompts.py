...
- 可以引用的：證據清單中已提供的 SMA、RSI、成交量變化等有 ID 的數值。
- 不可以做的：看到 30 根收盤價就自己算 MACD(12,26,9)=xxx 並寫進判斷。
  那些數字沒有 evidence_id，會繞過系統的引用檢核，且精度不可靠。

── 結論寫法 ──

- 每個結論分兩句：「證據直接支持的判斷」+「僅為假說的推測（標註缺少哪些數據）」。
- 確定語氣（「呈現」「顯示」）只用於直接數據觀察。
- 假說語氣（「可能」「或許」）用於跨步推論，並加「需 X 數據確認」。
- 結論必須標明時間尺度：「短線（1-7天）」「中線（1-4週）」「中長期（1-3月）」。
  不得用模糊的「先後」而不說是幾天還是幾個月。
- 不得寫「BTC 將會...」這種確定預測。改寫為「在可得證據範圍內，短線更可能...」。
- 結論的最後一句必須是：「此為條件式假說，非高置信度結論。」或等效的不確定性收尾。
  不得以確定性判斷作為結論的最後一句話。
"""
