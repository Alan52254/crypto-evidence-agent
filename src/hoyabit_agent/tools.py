...
def apply_contested_penalty(
    ...
    *,
    ...
    若不需要修正（無爭議判斷或判斷數不足），回傳原始 confidence 與空 dict。

    懲罰公式（連續函數，非斷崖式）：
    penalty = contested_ratio² × 0.40
    - 20% contested → 0.04² × 0.40 = 0.016 (幾乎不扣)
    - 40% contested → 0.16 × 0.40 = 0.064
    - 60% contested → 0.36 × 0.40 = 0.144
    - 80% contested → 0.64 × 0.40 = 0.256
    平方項讓低比例爭議幾乎不受影響，高比例爭議才顯著扣分。
    """
    if not isinstance(confidence, Confidence) or total_claims < 3:
        return confidence, {}

    contested_ratio = contested_count / total_claims
    if contested_ratio <= 0.0:
        return confidence, {}

    # 連續懲罰：penalty = ratio² × 0.40
    penalty = (contested_ratio ** 2) * 0.40
